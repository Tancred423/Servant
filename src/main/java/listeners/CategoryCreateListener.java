...
public class CategoryCreateListener extends ListenerAdapter {
    public void onCategoryCreate(@NotNull CategoryCreateEvent event) {
        ...
        CompletableFuture.runAsync(() -> {
            // Log
            if (myGuild.pluginIsEnabled("log") && myGuild.categoryIsEnabled("moderation") && myGuild.logIsEnabled("category_create")) {
                ...
                if (logChannel != null) {
                    logChannel.sendMessage(new EmbedBuilder()
                            ...
                            .setTitle(LanguageHandler.get(lang, "log_category_create_title"))
                            .addField(LanguageHandler.get(lang, "log_category_name"), event.getCategory().getName(), true)
                            .addField(LanguageHandler.get(lang, "category_id"), event.getCategory().getId(), true)
