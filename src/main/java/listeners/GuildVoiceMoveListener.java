    ...
    public void onGuildVoiceMove(@NotNull GuildVoiceMoveEvent event) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if (Toggle.isEnabled(event, "voicelobby")) {
                if (channels.contains(channel.getIdLong())) processVoiceLobby(event, guild, internalGuild, user, member, lang);
            }
        }, Servant.threadPool);
    }

    private static void processVoiceLobby(GuildVoiceMoveEvent event, net.dv8tion.jda.api.entities.Guild guild, Guild internalGuild, User user, Member member, String lang) {
