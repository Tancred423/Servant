...
import zJdaUtilsLib.com.jagrosh.jdautilities.command.AnnotatedModuleCompiler;
import zJdaUtilsLib.com.jagrosh.jdautilities.command.Command;
import zJdaUtilsLib.com.jagrosh.jdautilities.command.CommandBuilder;
import zJdaUtilsLib.com.jagrosh.jdautilities.command.CommandEvent;
import zJdaUtilsLib.com.jagrosh.jdautilities.command.annotation.JDACommand;
...
public class AnnotatedModuleCompilerImpl implements AnnotatedModuleCompiler {
    ...
    public List<Command> compile(Object o) {
        ...
        List<Method> commands = collect((Method method) -> {
            for(String name : module.value()) if (name.equalsIgnoreCase(method.getName())) return true;
            return false;
    ...
    private Command compileMethod(Object o, Method method) throws MalformedParametersException {
        ...
        // Aliases
        if(names.length>1) for(int i = 1; i<names.length; i++) builder.addAlias(names[i]);
        ...
        if(properties.children().length>0) {
            collect((Method m) -> {
                for(String cName : properties.children()) if (cName.equalsIgnoreCase(m.getName())) return true;
                return false;
    ...
    private static <T> List<T> collect(Predicate<T> filter, T... entities) {
        ...
        for(T entity : entities) if (filter.test(entity)) list.add(entity);
        return list;
    }
}
